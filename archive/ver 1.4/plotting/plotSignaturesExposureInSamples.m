...
%   colormap(jet);
  ...
  set(legend1,...
    ...
    'FontWeight','bold',...
    'FontSize',15);
  set(subplot1,'XTick', minGenomeSet:maxGenomeSet );
  set(subplot1,'XTickLabel',genomeNames(minGenomeSet:maxGenomeSet));
  rotateticklabel(subplot1, 90, 14);
  ...
  title('Normalized mutations in genomes');
    set(subplot2,'XTick', minGenomeSet:maxGenomeSet );
  set(subplot2,'XTickLabel',genomeNames(minGenomeSet:maxGenomeSet));
  rotateticklabel(subplot2, 90, 14);
  
  annotation(f1,'textbox',...
    [0.003 0.96 0.11 0.043],...
    'String',{[num2str(size(input.sampleNames, 1)) ' ' cancerType ' samples'], [num2str(minGenomeSet) ' to ' num2str(maxGenomeSet)], [ThousandSep(sum(sum(input.originalGenomes))) ' mutations']},...
